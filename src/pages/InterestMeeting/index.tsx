...
const InterestMeeting = ({
    ...
    function toggleInterested() {
        ...
        fetch(API_URL + "/user/update_interested", {
            ...
            body: JSON.stringify({
                interested: is_interested == "True" ? "False" : "True" as string,
            }),
        })
            .then((response) => {
                if (response.status === 200) {
                    setIsInterested(is_interested == "True" ? "False" : "True" as string);
                }
            })
            .catch((error) => {
                console.log(error);
                    ...
                    <>
                        {
                            is_interested && (
                                    ...
                                    <Button
                                        ...
                                        variant="contained"
                                        className={`${is_interested == "True" ? "" : "not-"}interested-button`}
                                        onClick={toggleInterested}
                                    >
                                        {/* image, depends on if the user is interested or not */}
                                        <img
                                            src={is_interested == "True" ? InterestedImage : NotInterestedImage}
                                            alt={`${is_interested ? "" : "Not"} interested`}
                                        />
                                        {/* text, depends on if the user is interested or not */}
                                        {`${is_interested == "True" ? "" : "Not"} Interested`}
                                    </Button>
                                )
                        }
                    </>
